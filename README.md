...
1. **Clone the repository**:
   ...
   ```bash
   git clone -b LAB3 https://github.com/pboon09/FRA502-LAB-6637.git
    ```
2. **Colcon build**:
   ```bash
   cd FRA502-LAB-6637/
...
2. **Use service call from RQT to call service**:
   ```bash
   ros2 run rqt_service_caller rqt_service_caller
   ```   

3. **Check if interface is already installed in machine**:
   ```bash
   ros2 interface show controller_interfaces/srv/SetParam
   ros2 interface show controller_interfaces/srv/SetMaxPizza
   ```   
- In `lab3/launch/lab3_bringup.launch.py`, you can change both turtle name and control frequency at
   ```
